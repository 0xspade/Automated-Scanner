...
if [ ! -f $1/$1-sublist3r.txt ]; then
	...
	curl -g "https://api.telegram.org/bot$telegram_bot/sendmessage?chat_id=$telegram_id&text=Sublist3r%20Found%20$sublist3rscan%20subdomain(s)%20for%20$1" --silent
	echo "[+] Done"
else
	curl -g "https://api.telegram.org/bot$telegram_bot/sendmessage?chat_id=$telegram_id&text=Skipping%20Sublist3r%20Scanning%20for%20$1" --silent
	echo "[!] Skipping ..."
fi
sleep 5
